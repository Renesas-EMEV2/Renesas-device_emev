...
status_t NV21JpegCompressor::compressRawImage(const void* image,
                                              ...
                                              int quality)
{
    ALOGD("%s: %p[%dx%d]", __FUNCTION__, image, width, height);
    ...
    if (encode(&mStream, pY, width, height, offsets, quality)) {
        ALOGD("%s: Compressed JPEG: %d[%dx%d] -> %d bytes",             __FUNCTION__, (width * height * 12) / 8, width, height, mStream.getOffset());
        return NO_ERROR;
    } else {
        ALOGD("%s: JPEG compression failed", __FUNCTION__);
