...
#if !defined(EMEV_PRCM_ENABLE)
static int WaitSeqBusy(void)
{
	int count = 0;
	
	while(1){
		if((readl(SMU_SEQ_BUSY) & EMXX_SMU_PG_SEQ_BUSY) == 0){
			break;
		}
		udelay(1);
		count++;
...
static void SetClkDiv(IMG_UINT32 val)
{
	emxx_close_clockgate(EMXX_CLK_A3D_SYS);
	emxx_close_clockgate(EMXX_CLK_A3D_MEM);
	emxx_close_clockgate(EMXX_CLK_A3D_CORE);

	writel(val, SMU_A3DCORECLKDIV);
}
#endif
...
#else
	emxx_open_clockgate(EMXX_CLK_A3D_SYS);
	emxx_open_clockgate(EMXX_CLK_A3D_MEM);
	emxx_open_clockgate(EMXX_CLK_A3D_CORE);
	
	writel(EMXX_SMU_PG_SWON_PD_ON, SMU_PG_SWON);
	
	if(WaitSeqBusy() != 0){
		PVR_DPF((PVR_DBG_ERROR, "EnableSGXClocks: Couldn't enable SGX functional clock"));
	}

	emxx_clkctrl_off(EMXX_CLKCTRL_A3DSYS);
	emxx_clkctrl_off(EMXX_CLKCTRL_A3DMEM);
	emxx_clkctrl_off(EMXX_CLKCTRL_A3DCORE);

	emxx_unreset_device(EMXX_RST_A3D);
#endif 
...
IMG_VOID DisableSGXClocks(SYS_DATA *psSysData)
{
	...
	emxx_reset_device(EMXX_RST_A3D);

	writel(EMXX_SMU_PG_SWON_PD_OFF, SMU_PG_SWON);

	if(WaitSeqBusy() != 0){
		PVR_DPF((PVR_DBG_ERROR, "DisableSGXClocks: Couldn't disable SGX functional clock"));
	}
...
PVRSRV_ERROR EnableSystemClocks(SYS_DATA *psSysData)
{
	...
	if (!psSysSpecData->bSysClocksOneTimeInit)
	{
		...
		if (IS_ERR(psCLK))
		{
			PVR_DPF((PVR_DBG_ERROR, "EnableSystemClocks: Couldn't get SGX Functional Clock"));
			goto ExitError;
		}
		psSysSpecData->psSGX_FCK = psCLK;
#else
		SetClkDiv(EMXX_SMU_CLK_DIV_A3D);
#endif 
