...
#define LOOP_COUNT 1
void checkandsetHDMI(int fd)
{
	enum EMXX_FB_OUTPUT_MODE output;

	struct scrn_modes screen;
	struct scrn_mode chk;
	int i;
	int fb_page;

	output = EMXX_FB_OUTPUT_MODE_HDMI_720P;
	if(ioctl(fd, EMXX_FB_SET_OUTPUT, &output)){
		LOGE("ioctl(EMXX_FB_SET_OUTPUT) failed!\n");
		return;
	}
	...
	for (i = 0; i < LOOP_COUNT; i++) {
		if(ioctl(fd, EMXX_FB_CHKSCRN, &chk) < 0){
			LOGV(" **TPfb: ioctl(EMXX_FB_CHKSCRN) error\n");
		} else {
			if(chk.page0 == 0){
				fb_page = 0;
			} else if(chk.page1 == 0) {
				...
				LOGE(" **TPfb: ioctl(EMXX_FB_CHKSCRN) timeout\n");
			}
		}
		...
		screen.rot       = 0;
		screen.update    = 1;
		ioctl(fd, EMXX_FB_UPDATE_SCRN, &screen);
	}
}
    ...
    if (!strcmp(name, GRALLOC_HARDWARE_FB0)) {
        ...
        if (status >= 0) {
            ...
            if (m->info.bits_per_pixel == 32) {
                if (m->info.red.offset == 16) { // ARGB8888
